...
void loop() {
  if (digitalRead(enabledPin) == LOW) {//disabled
    ...
    for (int n = 0; n < lct; n++) {
      if (n <= timeNum * lct) {//ON
        if (digitalRead(bluePin) == HIGH && digitalRead(autoPin) == HIGH) { //BLUE AND AUTO
          leds[n] = CHSV(170, 255, 100);
        }
        if (digitalRead(bluePin) == HIGH && digitalRead(autoPin) == LOW) { //BLUE AND TELE
          leds[n] = CHSV(170, 255, 200);
        }
        if (digitalRead(bluePin) == LOW && digitalRead(autoPin) == HIGH) { //RED AND AUTO
          leds[n] = CHSV(0, 255, 100);
        }
        if (digitalRead(bluePin) == LOW && digitalRead(autoPin) == LOW) { //RED AND TELE
          leds[n] = CHSV(0, 255, 200);
        }
      } else {//OFF
        if (digitalRead(bluePin) == HIGH && digitalRead(autoPin) == HIGH) { //BLUE AND AUTO
          leds[n] = CHSV(170, 255, 10);
        }
        if (digitalRead(bluePin) == HIGH && digitalRead(autoPin) == LOW) { //BLUE AND TELE
          leds[n] = CHSV(170, 255, 20);
        }
        if (digitalRead(bluePin) == LOW && digitalRead(autoPin) == HIGH) { //RED AND AUTO
          leds[n] = CHSV(0, 255, 10);
        }
        if (digitalRead(bluePin) == LOW && digitalRead(autoPin) == LOW) { //RED AND TELE
          leds[n] = CHSV(0, 255, 20);
        }
      }
    }
    FastLED.show();
